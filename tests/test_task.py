...
class TestTask:
    def test_task_is_displayed(self) -> None:
        ...
        assert Task(due_date=date(2022, 1, 5)).is_displayed(date(2022, 1, 6)) is True
        assert Task(due_date=date(2022, 1, 5), done=True).is_displayed(date(2022, 1, 6)) is False
        assert Task(due_date=date(2022, 1, 5), reminder_date=date(2022, 1, 3)).is_displayed(date(2022, 1, 1)) is False
        assert Task(due_date=date(2022, 1, 5), reminder_date=date(2022, 1, 3)).is_displayed(date(2022, 1, 3)) is True
        assert Task(due_date=date(2022, 1, 5), reminder_date=date(2022, 1, 3)).is_displayed(date(2022, 1, 4)) is True
        assert Task(due_date=date(2022, 1, 5), reminder_date=date(2022, 1, 3), done=True).is_displayed(date(2022, 1, 4)) is False

    def test_task_summary(self) -> None:
        # Task with only a due date
        task = Task(due_date=date(2022, 1, 5), title="Task 1")
        assert task.summary(today=date(2022, 1, 5)) == \
               "[**Due today**]"
        assert task.summary(today=date(2022, 1, 4)) ==\
               "[Due in 1 day]"
        assert task.summary(today=date(2022, 1, 1)) == \
               "[Due in 4 days]"
        assert task.summary(today=date(2022, 1, 6)) == \
               "[**Due 1 day ago**]"
        assert task.summary(today=date(2022, 1, 10)) == \
               "[**Due 5 days ago**]"
        ...
        assert task.summary(today=date(2021, 12, 31)) == \
